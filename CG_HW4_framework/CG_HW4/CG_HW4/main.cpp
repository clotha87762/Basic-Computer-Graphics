...
int isAutoRotating = 1;
int shaderType = 0; // 0 = Vertex ; 1 = Fragment;

int currentModel = 0;
#define NUM_OF_MODEL 17
...
void initTextures(int index)
{
	...
	// TODO: A Text2D texture is created there.
	 ...
	 glTexImage2D(GL_TEXTURE_2D,0,3,IH[index].Width,IH[index].Height,0,GL_RGB,GL_UNSIGNED_BYTE,image[index]); 
...
//	glmFacetNormals(OBJ);         
	//printf("%s\n",filename[index]);
	if(!strcmp(filename[index],"TextureModels/cube.obj")){
	//printf("qqq\n");
	glmFacetNormals(OBJ);    
...
void renderScene(void) 
{
	...
	Matrix4 NMVP;
	NMVP= (M.invert()).transpose();

	glUniformMatrix4fv(iLocNMVP,1,GL_FALSE,NMVP.get());
	...
	glUniform4fv(iLocLAmbient,1,Lambient);
		...
		glUniform1iv(iLocShader,1,&shaderType);
	...
	while(group){
		...
		glVertexAttribPointer(iLocTexCoord, 2,GL_FLOAT,GL_TRUE,0,vtextures[gCount]);
...
void processNormalKeys(unsigned char key, int x, int y) {
	...
	switch(key){
		...
		case 'P': case 'p':
			...
			break;
		case 'm':
			if(texture_min_filter == GL_NEAREST)
				texture_min_filter = GL_LINEAR;
			else if(texture_min_filter==GL_LINEAR)
				texture_min_filter = GL_NEAREST;
			break;
		case 'M':
			if(texture_mag_filter == GL_NEAREST)
				texture_mag_filter = GL_LINEAR;
			else if(texture_mag_filter==GL_LINEAR)
				texture_mag_filter = GL_NEAREST;
			break;
		case 'w':
		case 'W':
			if(texture_wrap_mode == GL_CLAMP)
				texture_wrap_mode = GL_REPEAT;
			else if(texture_wrap_mode == GL_REPEAT)
